   ...
   <div class="seccion">
    ...
    def _generar_comprobante(self, datos_comprobante):
        try:
            ...
            # --- 2. MAPEAR LOS DATOS DEL COMPROBANTE ---
            # Aquí debes adaptar los nombres de los campos según la estructura de tu BD.
            # Los 'get' con valores por defecto evitan errores si falta algún dato.
            datos_mapeados = {
                'logo_src': self._obtener_logo_base64(),  # Para logo en base64
                ...
                # --- Cálculo de montos ---
                # Asume que vienen del backend o los calcula aquí
                ...
                # --- Condiciones de pago ---
                'condicion_pago': datos_comprobante.get('condicion', 'Contado'),
            ...
            for idx, item in enumerate(items, 1):
                if isinstance(item, dict):
                    items_formateados.append({
                        ...
                        'unidad': item.get('um', 'NIU'),
                        'cantidad': float(item.get('cantidad', 1)),
                        'precio_unitario': float(item.get('valorVenta', 0)),
                        'total_item': float(item.get('cantidad', 1)) * float(item.get('valorVenta', 0))
                    })
                elif isinstance(item, (list, tuple)):
